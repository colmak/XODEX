...
func _on_optimize_pressed() -> void:
	var tower_panel_module: Variant = tower_selection_panel.get("module")
	if tower_panel_module == null:
		return
	var candidate: Dictionary = tower_panel_module.optimize_for_current_heat(global_heat_ratio)
