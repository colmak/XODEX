...
var thermal_profile: Dictionary = {
	"capacity": 100.0,
	"heat_per_shot": 18.0,
...
var map_mutation: Dictionary = DEFAULT_MAP_MUTATION.duplicate(true)

func mutate_for_pressure_cycle(factor: float) -> Dictionary:
	var next_profile: Dictionary = thermal_profile.duplicate(true)
...
func build_tower_thermal_profile() -> Dictionary:
	var profile: Dictionary = thermal_profile.duplicate(true)
...
func compute_enemy_pressure_speed(base_speed: float, efficiency: float, global_heat: float) -> float:
	var exponent: float = float(map_mutation.get("enemy_scaling_exponent", 1.0))
	var heat_gain: float = float(map_mutation.get("heat_feedback_gain", 0.18))
	var clamped_efficiency: float = clampf(efficiency, 0.2, 7.5)
	var escalation: float = pow(clamped_efficiency, exponent)
	var speed_scale: float = 1.0 + (escalation - 1.0) * 0.16 + global_heat * heat_gain
	speed_scale *= float(active_event.get("enemy_speed_multiplier", 1.0))
	return clampf(base_speed * speed_scale, base_speed * 0.85, base_speed * 2.4)
